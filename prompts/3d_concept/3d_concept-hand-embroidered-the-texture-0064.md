---
id: 3d_concept-hand-embroidered-the-texture-0064
title: "一生所爱刺绣专辑封面"
category: 3d_concept
subcategory: "04_创意脑洞特效合成"
aesthetic: null
tags: ["刺绣", "手工", "紫色", "国风", "专辑", "封面", "孙悟空", "大话西游"]
recommended_models: 
  - banana-pro
recommended_params:
  aspect_ratio: "1:1"
...
# 正文 prompt
Ultra-detailed album cover for \"{"song title" "歌曲名称"}\". Style: Huang Hai's minimalist aesthetic with HIGH-QUALITY HAND-EMBROIDERED SILK ART. Format: Vertical Poster (1:1 aspect ratio).
...
Visual Structure: 
- Delicate high-end silk fabric background with subtle sheen and fine weave texture, {"color scheme" "[配色方案]"}, occupying 75% of canvas.
- Scene Visualization: A simplified, poetic scene of {"scene description" "[场景描述]"}, rendered in hand-embroidered silk with VISIBLE REALISTIC STITCHING DETAILS.
...
Line 1: \"{"first lyric line" "[第一句歌词]"}\"\nLine 2: \"{"second lyric line" "[第二句歌词]"}\"\nRendered in elegant [书法风格] style calligraphy with refined strokes, all hand-embroidered with VISIBLE INDIVIDUAL STITCHES. The text should show clear hand-stitched style with spacing between stitches. [音乐风格] musical notes arranged in subtle [图案] patterns. Embroidered in [主色线] and [辅色线] threads, connecting notes and text with delicate [线条类型] lines, showing OBVIOUS hand-embroidered texture with individual stitch visibility.

Border: Minimal single thin line border in [边框颜色] (hand-embroidered). Corner: One small elegant [符号] symbol (hand-stitched

## 中文译文（源站提供）
超精细专辑封面，主题为“{"song title" "Song Title"}”。风格：黄海的极简主义美学，采用高品质手绣丝绸艺术。格式：竖版海报（1:1 宽高比）。

关键：每个元素都必须呈现手绣效果，带有清晰可见的逼真缝线细节。展示独立的线迹、轻微的纹理变化和地道的刺绣工艺。

视觉结构：
- 精致的高端丝绸面料背景，带有微妙的光泽和精细的编织纹理，{"color scheme" "[Color Scheme]"}，占据画布的 75%。
- 场景可视化：一个简化、富有诗意的场景，描绘了 {"scene description" "[Scene Description]"}，以手绣丝绸呈现，带有清晰可见的逼真缝线细节。
- 比例对比：在底部/角落有微小、几乎不可见的 [Detail Elements] 剪影。

底色：奢华丝绸面料背景，带有微妙的光泽、精细的编织纹理和柔和的光泽感。

手绣细节：每个元素都必须展示：
- 独立的可见线迹，带有轻微的纹理
- 微观丝线细节，带有微妙的金属光泽
- 逼真的刺绣工艺，带有轻微不完美、自然的缝线
- 可见的缝线图案和线材纹理变化
- 真实的手绣外观，带有自然的瑕疵
场景采用柔和的低饱和度色彩（[Element] 为浅 [Color]，[Element] 为柔和 [Color]，[Element] 为柔和 [Color]）。真实感：极强的立体深度、逼真的手绣缝线、带有可见缝线细节的触感丝绸纹理。

调色板：微妙的高端 [Main Color] 丝绸背景。场景采用柔和的低饱和度色彩（浅 [Color]、柔和 [Color]、柔和 [Color]）。营造优雅、精致、考究的氛围。

排版：
- 顶部标题“{"song title" "Song Title"}”采用极简优雅的衬线字体（以微妙的线材手绣），周围留有最大呼吸空间。
- 副标题“[Artist] • [Year] • [Genre]”采用精致字体。
- 小号双语文本“[English Keyword] [Chinese Keyword]”采用精致字体。

关键 - 底部设计：文本必须呈现明显的手绣风格，具有：
- 可见的独立缝线间距
- 有机、略带不完美的缝线图案
- 线材自然的纹理变化
- 手工缝制的外观，而非密集的机器刺绣
...
底部设计：优雅流畅的音符与精致的书法交织，展示歌曲中两句连贯、富有叙事性的歌词：
第一行：“{"first lyric line" "[First Lyric Line]"}"\n第二行：“{"second lyric line" "[Second Lyric Line]"}"\n以优雅的 [Calligraphy Style] 书法风格呈现，笔触精致，全部手绣，带有清晰可见的独立缝线。文本应呈现清晰的手工缝制风格，缝线之间有间距。[Music Style] 音符以微妙的 [Pattern] 图案排列。以 [Main Thread Color] 和 [Accent Thread Color] 线材刺绣，用精致的 [Line Type] 线条连接音符和文本，呈现明显的手绣纹理，并可见独立的缝线。

边框：[Border Color] 的极简单细线边框（手绣）。角落：一个小的优雅 [Symbol] 符号（手绣）。
