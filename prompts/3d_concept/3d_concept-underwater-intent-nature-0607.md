---
id: 3d_concept-underwater-intent-nature-0607
title: "生成水下自然生态海报"
category: 3d_concept
subcategory: "04_创意脑洞特效合成"
aesthetic: biophilic
tags: ["海龟", "珊瑚", "海底", "鱼群", "海洋", "蓝色", "自然", "静谧"]
recommended_models: []
recommended_params:
  aspect_ratio: "1:1"
  quality: null
  style_strength: null
sample_outputs: []
quality_tier: B
author: "ChunqiGuo02"
source: "Leader Prompt Gallery (https://www.leaderai.top/mid-api/lab/image_prompt/index.html)"
language: en
created_at: 2026-05-30
version: "1.0.0"
related_to: []
---

# 正文 prompt
Intent: nature poster. Background: clear turquoise water fading to deeper blue. Foreground: vibrant coral clusters with small fish. Hero subject: sea turtle swimming across the center. Finishing details: underwater light rays, natural color grading, no logos or trademarks, no watermark. Camera: wide-angle, underwater eye-level.

## 中文译文（源站提供）
意图：制作一张关于自然环境的海报。  
背景：清澈的蓝绿色海水逐渐过渡为更深的蓝色。  
前景：色彩鲜艳的珊瑚群以及其中游动的小鱼。  
主体：一只海龟在画面中央游动。  
细节处理：添加水下光线效果，采用自然色彩渐变处理；海报上不得出现任何标志、商标或水印。  
拍摄设备：广角镜头，从水下与人眼平行的角度进行拍摄。

## 来源备注
- 源站作者：官方演示案例
- 源站 UUID：acNln6BhxI
- 源站分类：04_创意脑洞特效合成
