---
id: 3d_concept-the-ingredients-page-0991
title: "鸡肉培根全球美食图鉴"
category: 3d_concept
subcategory: "04_创意脑洞特效合成"
aesthetic: null
tags: ["复古", "手绘", "插画", "立体", "暖色调", "美食", "烹饪", "鸡肉"]
recommended_models: 
  - banana-pro
recommended_params:
  aspect_ratio: "1:1"
...
# 正文 prompt
<instruction>
Inputs: Ingredients = [{"ingredient 1" "Chicken"}, {"ingredient 2" "Bacon"}]
...
## 中文译文（源站提供）
<instruction>
输入：食材 = [{参数 名称="食材 1" 默认值="鸡肉"}, {参数 名称="食材 2" 默认值="培根"}]

步骤 1：全球分析
分析这些食材。
选择 4 个以这些食材制作著名菜肴的不同国家。

目标：一个“烹饪地图集”网格，展示这两种食材的 4 种不同文化变体。

布局规则：
输出格式：一个 2x2 的网格。
每个面板内容：这 4 个面板中的每一个都必须展示一本针对特定国家的单页打开的书籍。

每个面板内部（书籍展开规则）：
左页（历史）：2D 复古棕褐色素描。该菜肴的视觉时间线或插图食谱（例如，“1850 年 -> 1920 年 -> 今天”）。扁平的墨线画风格。
右页（现实）：3D 立体模型。成品、热气腾腾的餐点从页面中实体浮现。来自该国的一位迷你厨师站在页面上，旁边是巨大的食物。
一致性：所有 4 本书看起来都像是同一系列的一部分，但内容具有不同的文化特色。

灯光：电影般的俯视灯光，突出扁平左页和 3D 右页之间的深度差异。
输出：2x2 网格，高清晰度，宏观细节。
</instruction>
