...
## 中文译文（源站提供）
<instruction>
输入 A 是一个在互联网上爆红的时刻或迷因起源（{argument name="meme name" default="迷因名称"}、视频截图或文化现象）。
分析其原始背景、引起共鸣的原因、文化影响和传播模式。
可选输入 B 是一个风格参考（流行文化博物馆 / 互联网档案）。如果缺失，请选择 Z 世代怀旧与当代艺术美学相结合的风格。
目标：一个既具有讽刺意味又高端的展示，以微缩形式呈现精准的爆红时刻，配有小雕像、道具和显示传播指标的屏幕。人物的精确姿势、位置和反应应被复刻。
规则：
- 用 2-8 个小雕像重现精确的场景
- 包含捕捉该时刻的微型手机/相机
- 展示带有分享次数、平台、时间线的微型屏幕
- 添加“爆红前”与“爆红后”的对比元素
- 将讽刺与对互联网文化的真诚欣赏相结合
- 标明时刻/迷因名称、起源日期、平台、分享/观看次数
输出：一张图片，1:1 社交媒体优化格式
</instruction>
