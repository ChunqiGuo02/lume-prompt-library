---
id: product-comic-book-toy-0545
title: "漫画3D立体书场景生成提示词"
category: product
subcategory: "12_手工玩具手办"
aesthetic: null
tags: ["3D", "美漫", "手办", "立体", "场景", "超级英雄", "超人", "蝙蝠侠"]
recommended_models: 
  - banana-pro
recommended_params:
  aspect_ratio: "1:1"
  quality: null
  style_strength: null
sample_outputs: []
quality_tier: A
author: "ChunqiGuo02"
source: "Leader Prompt Gallery (https://www.leaderai.top/mid-api/lab/image_prompt/index.html)"
language: en
created_at: 2026-05-30
version: "1.0.0"
related_to: []
---

# 正文 prompt
2x2 grid, do this for 4 titles in a famous comic universe: <spatial_map>
    [ VERTICAL SCENIC BACKDROP (BOOK COVER) ]
    |      (Floating Label: [【ISSUE_TITLE】 | 【YEAR】])
    |                                            |
    |      [3D TOY: 【HERO】] vs [3D TOY: 【VILLAIN】]   |
    |          \__ [3D PLASTIC FX] __/           |
    |                                            |
 ---+--------------------------------------------+---
   /    [OPEN BOOK PAGES WITH COMIC PANELS]     /
  /                                            /
 /      [ FLOATING 3D TEXT: [SFX_WORD]! ]     /
/____________________________________________/
</spatial_map>
Instruction: Render a photorealistic 3D diorama matching this exact structural geometry.

## 中文译文（源站提供）
2x2 网格，为著名漫画宇宙中的 4 个标题执行此操作：<spatial_map>
    [ 垂直风景背景（图书封面） ]
    |      (浮动标签：[【ISSUE_TITLE】 | 【YEAR】])
    |                                            |
    |      [3D 玩具：【HERO】] 对战 [3D 玩具：【VILLAIN】]   |
    |          \__ [3D 塑料特效] __/           |
    |                                            |
 ---+--------------------------------------------+---
   /    [ 翻开的漫画书页 ]     /
  /                                            /
 /      [ 浮动 3D 文本：[SFX_WORD]! ]     /
/____________________________________________/
</spatial_map>
说明：渲染一个与此精确结构几何相符的超逼真 3D 立体模型。

## 来源备注
- 源站作者：Gadgetify
- 源站 UUID：pUkg8Ivv7n
- 源站分类：12_手工玩具手办
