---
id: "product-the-cute-character-001"
title: "Q版潮男盲盒人偶"
category: product
subcategory: "12_手工玩具手办"
aesthetic: null
tags: ["3D", "Q版", "可爱", "盲盒", "男性", "眼镜", "短发", "西装"]
recommended_models: 
  - banana-pro
recommended_params:
  aspect_ratio: "3:4"
  quality: null
  style_strength: null
sample_outputs: []
quality_tier: B
author: "ChunqiGuo02"
source: "Leader Prompt Gallery (https://www.leaderai.top/mid-api/lab/image_prompt/index.html)"
language: en
created_at: 2026-05-30
version: "1.0.0"
related_to: []
---

# 正文 prompt
Transform the person in the uploaded photo into a cute 3D Pop Mart style blind box character . Likeness : Keep key features recognizable: [hair color, glasses, hairstyle]. Style : C4D rendering, occlusion render, cute Q-version , soft studio lighting, pastel colors. Background : A simple, solid matte color background (e.g., soft blue). Detail : The character should have a smooth, plastic toy texture with a slight glossy finish. Facing forward, friendly expression.

## 中文译文（源站提供）
将上传照片中的人物转换为可爱的3D Pop-Mart风格的盲盒角色。相似性：保持关键特征的可识别性：[头发颜色、眼镜、发型]。风格：C4D渲染、遮挡渲染、可爱的Q版、柔和的工作室照明、柔和的色彩。背景：简单、纯色的哑光背景（例如，柔和的蓝色）。细节：角色应该具有光滑的塑料玩具质感，表面有轻微的光泽。面向前方，友好的表情。

## 来源备注
- 源站作者：WeChat Article
- 源站 UUID：1d6309b1ba
- 源站分类：12_手工玩具手办
