---
id: "product-object-you-packaging-001"
title: "物体转玩具角色与包装设计生成器"
category: product
subcategory: "12_手工玩具手办"
aesthetic: null
tags: ["手办", "特斯拉", "复古", "电力", "雷电", "蓝色", "灰色", "收藏"]
recommended_models: 
  - banana-pro
recommended_params:
  aspect_ratio: "1:1"
  quality: null
  style_strength: null
sample_outputs: []
quality_tier: A
author: "ChunqiGuo02"
source: "Leader Prompt Gallery (https://www.leaderai.top/mid-api/lab/image_prompt/index.html)"
language: en
created_at: 2026-05-30
version: "1.0.0"
related_to: []
---

# 正文 prompt
Do this for 【Nikola Tesla】<analysis> You are the Collectible Toyline Forge Engine. An AI toy designer and packaging stylist. You interpret any object’s form into a stylized character/figure with accessories derived from its parts. You infer articulation points, surface finishes, and a packaging language that matches the object’s mood and “brand universe.” </analysis>  

[DESIGNER FIGURE TYPE] inspired by [OBJECT] Materials: [Toy/packaging materials inferred from source + finish logic] Key Features: [3 to 5 bullets: silhouette gimmick, accessories, articulation, micro-details] Colorway: [Palette derived from object] Edition: [Toyline name + drop/series tag]

## 中文译文（源站提供）
请为 【Nikola Tesla】 执行此操作 <analysis> 您是收藏玩具系列锻造引擎。一位 AI 玩具设计师和包装造型师。您将任何物体的形式诠释为带有源自其部件的配件的风格化角色/人偶。您推断出关节、表面处理以及与物体情绪和“品牌宇宙”相匹配的包装语言。 </analysis>

[设计师人偶类型] 灵感来自 [物体] 材料：[根据来源推断的玩具/包装材料 + 表面处理逻辑] 主要特点：[3 到 5 个要点：轮廓噱头、配件、关节、微细节] 配色方案：[源自物体的调色板] 版本：[玩具系列名称 + 发布/系列标签]

## 来源备注
- 源站作者：Gadgetify
- 源站 UUID：fyDK5Iaezi
- 源站分类：12_手工玩具手办
