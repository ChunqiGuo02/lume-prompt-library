---
id: "product-tiny-hyperrealistic-model-scenery-001"
title: "3d 超写实场景模型"
category: product
subcategory: "02_电商产品虚拟摄影"
aesthetic: biophilic
tags: ["超写实", "夜景", "弯月", "阶梯", "绿植", "蘑菇", "女子", "梦幻"]
recommended_models: 
  - banana-pro
recommended_params:
  aspect_ratio: "3:4"
  quality: null
  style_strength: null
sample_outputs: []
quality_tier: B
author: "ChunqiGuo02"
source: "Leader Prompt Gallery (https://www.leaderai.top/mid-api/lab/image_prompt/index.html)"
language: en
created_at: 2026-05-30
version: "1.0.0"
related_to: []
---

# 正文 prompt
3d hyperrealistic model-scenery of a long-haired woman wearing a flowing maxi dress walking up a curved staircase with an arched doorway at the end with a view to a tiny glowing moon, framed by lush climbing vines, setting over a round base made out of moss and tiny bioluminescent mushrooms, ethereal lighting, staging aesthetics, dark plain background --ar 4:5 --c 6 --s 100

## 中文译文（源站提供）
这是一个3D超写实模型场景：一位长发女子穿着飘逸的长裙，沿着弯曲的楼梯向上走去。楼梯尽头是一个拱形的门，门外可以看到一轮微小的发光月亮。月亮被茂密的攀缘植物环绕，而整个场景则矗立在一个由苔藓和发光蘑菇构成的圆形基座上，营造出一种梦幻般的氛围。

## 来源备注
- 源站作者：PatriciaRed_
- 源站 UUID：3248c5d52a
- 源站分类：02_电商产品虚拟摄影
