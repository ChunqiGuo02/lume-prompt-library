---
id: "product-the-studio-box-001"
title: "机娘模型包装盒设计展示"
category: product
subcategory: "12_手工玩具手办"
aesthetic: null
tags: ["机娘", "模型", "包装", "机甲", "二次元", "黄色", "黑色", "棚拍"]
recommended_models: 
  - banana-pro
recommended_params:
  aspect_ratio: "3:4"
...
## 中文译文（源站提供）
subject: >
一张 1/7 比例塑料模型的实体纸盒的影棚照片。
该纸盒在拍摄前已完全印刷。
正面仅作为纯粹的包装艺术面板，而非信息展示面。
...
全出血，无边框，无任何额外元素。
照片中仅可见一个侧面板。
这个可见的侧面板包含一个结合了参考图像 A 和参考图像 B 的布局，
以及组装好的模型示例、特写细节视图、武器配件，
和像真实商业塑料模型包装一样排列的注意事项。
另一个侧面板背对镜头，不可见。
...
text: >
正面：零生成文本，零警告，零符号，零标签。
正面唯一可见的文本是已嵌入参考图像 C 中的文本。
所有 g
