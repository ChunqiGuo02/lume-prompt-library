---
id: "product-armor-reactor-visible-001"
title: "机甲透视热力图生成器"
category: product
subcategory: "04_创意脑洞特效合成"
aesthetic: industrial-loft
tags: ["机甲", "机器人", "透视图", "X光", "热成像", "科幻", "机械", "发光"]
recommended_models: 
  - banana-pro
recommended_params:
  aspect_ratio: "3:4"
...
# 正文 prompt
Nano Banana Proで、ロボットキャラのX線／CTスキャン風の透過図+エネルギー源ヒートマップっぽい描画
プロンプト置いておきます。AIに食わせて好きなロボやメカにアレンジすると良いです☺️
---
...
"(battle-worn heavy armor, heat-distorted metal):1.25"

## 中文译文（源站提供）
使用Nano Banana Pro软件，可以生成类似机器人角色X光/CT扫描的透明图像，同时搭配类似能量源热图的渲染效果。  
我已准备好相关提示：你可以将这些素材输入AI模型，让AI根据喜好进行调整或修改，从而创造出自己理想的机器人或机械形态☺️  

---

**主题：** “Armored Trooper ATM-09-ST Scopedog”——源自《Armored Trooper VOTOMS》的经典第一代装甲战士  
**视角：** 严密的正面视角，完美的正交投影效果，无任何透视变形，结构完全对称。  
**风格：** 高对比度的军事工业风格渲染，呈现冷战时期的武器实验室场景。  
**背景：** 纯黑虚空，绝对真空环境，无反射、无地面、无阴影（仅显示自身发出的光线）。  
**美学特征：** 使用伪彩色技术模拟X光成像效果，并展现出明显的机械磨损痕迹。  
**透明度设置：**  
  - **半透明层**：厚重的装甲板与轧制的均质钢复合材料。  
  - **透明层**：内部骨架和系统结构轮廓极为清晰细致。  
**内部组件：**  
  - 安装在中央躯干处的定制等离子聚变反应堆。  
  - 暴露在外的液压肌肉缸和重型线性执行器。  
  - 充满高温冷却液的高压流体管道。  
  - 通过弹链供弹的30毫米 Gatling 步枪，可清晰观察到弹药流动情况。  
  - 安装在尾部裙部的强力火箭推进器。  
  - 密布的红外/光学传感器及镜头塔。  
  - 驾驶舱内有明显的飞行员座椅和控制杆。  
  - 关节处的电线和冷却管道清晰可见。  
**热图渐变：**  
  - **基础颜色**：深冷蓝色与枪金属灰色（非活动装甲区域）。  
  - **中间颜色**：暗绿色至灼烧橙色（液压系统区域）。  
  - **高亮颜色**：发亮的黄色至熔融红色（反应堆核心、推进器及枪管）。  
  - **最高亮度**：反应堆屏蔽层及超压部位的炽热白色。  
**光照效果：**  
  - **主要光源**：来自反应堆排气口的强烈蓝白色等离子光。  
  - 其他光源：  
    - 从装甲缝隙中透出的反应堆光芒。  
    - 红热的液压管线。  
    - 镜头塔上的琥珀色警告灯。  
    - 火箭喷嘴散发出的熔融橙色光。  
**其他细节：**  
  - 真实的战斗损伤痕迹（弹坑、烧焦痕迹、油漆剥落）。  
  - 可见的焊缝、铆钉及维护舱口。  
  - 附着尘垢的工业质感、油污及液压液体泄漏痕迹。  
**质量优化选项：**  
  - 8K分辨率，极强的机械真实感。  
  - 极高的工程精度（相当于Yamato 1/12比例模型套件的细致程度）。  
  - 军事技术手册风格的渲染效果，充满冷酷质感。  
  - 光照效果具有高度写实感，微小划痕也能清晰显示。  

**负面提示（避免使用的情况）：**  
  - 卡通风格、夸张的动漫元素；过于干净整洁的玩具外观；发光的眼睛；飞行员面部清晰可见；绿色机械体；《Gundam》系列的部件；光线效果；LOGO；文字或水印；多余肢体；明显的透视失真；明亮的背景；过于可爱的造型或变形明显的模型。  

**评分参考标准：**  
  - **杰作（超高细节、极致真实感）：1.4**  
  - “Armored Trooper Scopedog”（完美比例、经典VOTOMS设计）：1.6  
  - 军事工业风格的X光热成像效果、强烈的机械真实感：1.35  
  - 经过战斗磨损的厚重装甲、因高温而变形的金属表面：1.25
