---
id: product-the-cute-round-0706
title: "钩针编织可爱玩偶特写，柔和光影，毛线纹理清晰"
category: product
subcategory: "12_手工玩具手办"
aesthetic: null
tags: ["可爱", "钩针", "玩偶", "手工", "蓝色", "浅色系", "治愈", "暖光"]
recommended_models: 
  - banana-pro
recommended_params:
  aspect_ratio: "1:1"
  quality: null
  style_strength: null
sample_outputs: []
quality_tier: B
author: "ChunqiGuo02"
source: "Leader Prompt Gallery (https://www.leaderai.top/mid-api/lab/image_prompt/index.html)"
language: en
created_at: 2026-05-30
version: "1.0.0"
related_to: []
---

# 正文 prompt
A cute, round, and fluffy amigurumi (crocheted stuffed toy) of a {argument name="subject" default="cat"}. The texture should clearly show the yarn and crochet stitches. Soft, warm lighting. Pastel colors. Close-up shot with shallow depth of field. High quality, detailed.

## 中文译文（源站提供）
一只可爱、圆润、毛茸茸的钩针玩偶（amigurumi），是{argument name="subject_en" default="猫"}的形象。纹理应清晰展现毛线和钩针针法。柔和、温暖的灯光。马卡龙色调。特写镜头，景深较浅。高质量，细节丰富。

## 来源备注
- 源站作者：ちゃろん
- 源站 UUID：2SdxGGKLgp
- 源站分类：12_手工玩具手办
