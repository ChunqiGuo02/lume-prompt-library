---
id: "product-brand-the-commercial-001"
title: "打造高燃品牌广告大片"
category: product
subcategory: "03_创意广告品牌设计"
aesthetic: null
tags: ["口红", "彩妆", "红色", "粒子", "光影", "写实", "三维", "广告"]
recommended_models: 
  - banana-pro
recommended_params:
  aspect_ratio: "16:9"
  quality: null
  style_strength: null
sample_outputs: []
quality_tier: A
author: "ChunqiGuo02"
source: "Leader Prompt Gallery (https://www.leaderai.top/mid-api/lab/image_prompt/index.html)"
language: en
created_at: 2026-05-30
version: "1.0.0"
related_to: []
---

# 正文 prompt
Cinematic 3D action-packed advertisement for [INSERT PRODUCT/BRAND HERE], captured in an intense mid-motion moment with dramatic studio lighting, dynamic particle effects, and high-impact slow-motion energy. Ultra-hyperrealistic rendering, razor-sharp details, glossy commercial finish, atmospheric depth, and powerful contrast. Viral-ready composition with the brand logo seamlessly integrated into the scene and a sleek, modern slogan positioned cleanly beneath. High-end blockbuster commercial aesthetic

## 中文译文（源站提供）
为 [在此处插入产品/品牌名称] 制作一部充满电影感的 3D 动作广告，捕捉产品在中动作状态下的精彩瞬间；运用强烈的摄影棚灯光、动态的粒子效果以及极具冲击力的慢动作效果。广告画面需具备超写实的渲染效果、极其清晰的细节、光滑的视觉质感、丰富的空间层次感以及强烈的对比度。整体构图应适合在社交媒体上广泛传播，品牌标志需自然融入场景中，同时简洁现代的口号需清晰地展示在画面下方。整个广告应呈现出高端大片般的视觉风格。

## 来源备注
- 源站作者：Amira Zairi
- 源站 UUID：I6uEL7q1eT
- 源站分类：03_创意广告品牌设计
