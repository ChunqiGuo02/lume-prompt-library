---
id: architecture-the-silhouette-sony-0185
title: "伊斯坦布尔黄昏垂钓剪影"
category: architecture
subcategory: "15_特定场景环境生成"
aesthetic: null
tags: ["剪影", "日落", "渔民", "桥梁", "清真寺", "海鸥", "暖色调", "橙色"]
recommended_models: 
  - banana-pro
recommended_params:
  aspect_ratio: "3:2"
  quality: null
  style_strength: null
sample_outputs: []
quality_tier: A
author: "ChunqiGuo02"
source: "Leader Prompt Gallery (https://www.leaderai.top/mid-api/lab/image_prompt/index.html)"
language: en
created_at: 2026-05-30
version: "1.0.0"
related_to: []
---

# 正文 prompt
GPT Image 1.5 (sol) vs Nano Banana Pro (sağ)
Prompt: A cinematic silhouette shot of local fishermen lining the Galata Bridge in Istanbul during sunset. In the foreground, a weary fisherman leans against the railing, looking at the sea. The background features the hazy, dreamlike silhouette of the Süleymaniye Mosque and seagulls flying over the Golden Horn. Warm orange and deep blue color palette. Shot on Sony A7R IV with Sony FE 85mm f/1.4 GM lens at f/1.8. Atmospheric haze, emotional storytelling, award-winning travel photography.

## 中文译文（源站提供）
这张照片特写了一群当地渔民在伊斯坦布尔加拉塔大桥旁夕阳下站立的剪影。前景中，一位疲惫的渔夫靠在栏杆上，凝视着大海。背景中，苏莱曼尼耶清真寺朦胧的轮廓若隐若现，海鸥在金角湾上空飞翔。整体色彩以温暖的橙色调和深蓝色为主。使用索尼A7R IV相机和索尼FE 85mm f/1.4 GM镜头（镜头光圈设定为f/1.8）拍摄。照片具有浓厚的艺术氛围、深刻的叙事力量，堪称获奖级别的旅行摄影作品。

## 来源备注
- 源站作者：Ozan Sihay
- 源站 UUID：8562029beb
- 源站分类：15_特定场景环境生成
