---
id: "architecture-the-prompt-elements-001"
title: "城市街景微缩模型制作"
...
## 中文译文（源站提供）
根据上传的图片，制作一个超逼真的等轴测立体模型。分析图片以提取主要的建筑风格、建筑年代、建筑材料、街道布局、物体、车辆和城市密度。将同一场景重新构建为一个浮在纯白色背景上的单个城市街区。在将其转换为微型建筑模型时，要保持原有的氛围、比例和空间逻辑。如果图片中有中层建筑，请复现其外观质感、阳台、窗户、店面和街道元素。仅保留原图片中可见的元素，去除任何不存在的元素。应用45度等轴测视角、倾斜偏移的缩略图效果、与原始光照条件相匹配的柔和自然日光效果、全局光照效果、PBR材质以及极高的细节精度。照明效果要干净整洁（无需天空或地平线元素）。"
