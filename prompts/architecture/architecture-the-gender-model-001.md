---
id: "architecture-the-gender-model-001"
title: "高定时尚魔毯飞行大片"
category: architecture
subcategory: "05_人物肖像与写实摄影"
aesthetic: null
tags: ["女性", "飞毯", "时尚", "暖色调", "光影", "建筑", "沙尘", "高级感"]
recommended_models: 
  - banana-pro
recommended_params:
  aspect_ratio: "16:9"
  quality: null
  style_strength: null
sample_outputs: []
quality_tier: A
author: "ChunqiGuo02"
source: "Leader Prompt Gallery (https://www.leaderai.top/mid-api/lab/image_prompt/index.html)"
language: en
created_at: 2026-05-30
version: "1.0.0"
related_to: []
---

# 正文 prompt
{
  "options": {
    "gender": ["man", "woman"]
  },
  "prompt": "A high-fashion editorial shot of a {"gender" "woman"} model sitting or standing on a flying magic carpet during takeoff. The model is wearing a sophisticated, layered outfit featuring the structured silhouettes and premium textures typical of luxury brands like Celine, Balenciaga, Ralph Lauren, Givenchy, and Zegna. The clothes billow elegantly in the wind. The carpet is ornate and detailed, kicking up a subtle dust cloud as it ascends from a sun-drenched, minimalist architectural terrace. The lighting is cinematic and warm, highlighting the motion and the luxurious fabrics. Low angle perspective focusing entirely on the subject and the scene."
}

## 中文译文（源站提供）
```json
{
  "options": {
    "gender": ["男", "女"]
  },
  "prompt": "一张高级时装编辑照片，一名{\"gender\" \"女\"}模特坐或站在一张正在起飞的飞毯上。模特身着精致的叠穿服装，其结构化的廓形和优质的纹理是 Celine、Balenciaga、Ralph Lauren、Givenchy 和 Zegna 等奢侈品牌的典型特征。衣服在风中优雅地飘动。飞毯华丽而精致，从阳光普照的极简主义建筑露台升空时，扬起一丝细微的尘土。灯光富有电影感且温暖，突出了动感和奢华面料。低角度视角完全聚焦于主体和场景。"
}
```

## 来源备注
- 源站作者：Lloyd Creates
- 源站 UUID：9Gekvs1UgN
- 源站分类：05_人物肖像与写实摄影
