---
id: ip_design-the-character-image1-0050
title: "动漫女孩谈论布丁"
category: ip_design
subcategory: "04_动漫游戏视觉与IP改编"
aesthetic: null
tags: ["Seedance 2.0", "动漫女孩", "甜品", "口型同步", "中景", "低角度", "明亮温暖", "室内场景"]
recommended_models: []
recommended_params:
  aspect_ratio: "16:9"
  quality: null
  style_strength: null
sample_outputs: []
quality_tier: B
author: "ChunqiGuo02"
source: "Leader Prompt Gallery (https://www.leaderai.top/mid-api/lab/image_prompt/index.html)"
language: en
created_at: 2026-05-30
version: "1.0.0"
related_to: []
---

# 正文 prompt
The character from <<<Image1>>>   is an anime-style girl talking enthusiastically about pudding. She speaks with the voice and timing from ♪<<<Audio1>>>   , with natural lip-sync matching the audio.
Medium shot, slightly low angle, facing the camera. Bright, warm indoor

## 中文译文（源站提供）
来自 <<<Image1>>> 的角色是一位动漫风格的女孩，热情地谈论布丁。她以 ♪<<<Audio1>>> 的声音和节奏说话，并与音频自然同步口型。  
中景，略低角度，对着摄像机。明亮、温暖的室内环境

## 来源备注
- 源站作者：ビームマンＰ ver40
- 源站 UUID：nx6qx59us6
- 源站分类：04_动漫游戏视觉与IP改编
