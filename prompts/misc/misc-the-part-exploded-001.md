---
id: "misc-the-part-exploded-001"
title: "任意物体的轴测分解图"
category: misc
subcategory: "08_图像分析信息拆解"
aesthetic: null
tags: ["爆炸图", "发动机", "机械", "零件", "工程图", "示意图", "标注", "技术"]
recommended_models: 
  - banana-pro
recommended_params:
  aspect_ratio: "3:4"
...
## 中文译文（源站提供）
[被拆解物体的名称] 的等轴测分解图。沿一条清晰的垂直轴线将每个部件精确地分开，并保持适当的间距。每个部件的渲染都要保证比例准确、几何形状清晰。图中应包含：外壳、内部框架、机械零件、电气模块、印刷电路板（PCB）、芯片、连接器、按钮、开关、传感器、电池以及螺丝。使用中性色调的灯光和纯白色背景。添加细而均匀的标注线，指向各个部件，并在图像周围整齐地放置简短易读的标签。禁止使用任何艺术化的表现手法，不得过度夸大材质的质感或出现变形。整个图示应呈现出清晰、专业的工程分解效果，类似于硬件维修手册的风格。
