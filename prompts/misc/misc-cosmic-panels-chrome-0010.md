---
id: misc-cosmic-panels-chrome-0010
title: "Cosmic Swagger：哈哈，高达的设计图终于公开了！"
category: misc
subcategory: "08_图像分析信息拆解"
aesthetic: null
tags: ["机甲", "蓝图", "科幻", "复古", "青色", "深蓝", "机械", "设计图"]
recommended_models: 
  - banana-pro
recommended_params:
  aspect_ratio: "16:9"
  quality: null
  style_strength: null
sample_outputs: []
quality_tier: A
author: "ChunqiGuo02"
source: "Leader Prompt Gallery (https://www.leaderai.top/mid-api/lab/image_prompt/index.html)"
language: en
created_at: 2026-05-30
version: "1.0.0"
related_to: []
---

# 正文 prompt
style chest panels with warm glow
Chunky chrome toggles and rotary dials on the cockpit hatch
Retro starship-style backpack thrusters, with magenta and teal plasma vents
Arm-mounted “Funk Cannons” (Captain HaHaa signature)
Legs with mirrored chrome panels reflecting cosmic colours
Foot stabilizers shaped like mini hover-skids
10 labels describing each unique feature:

HaHaa-Crest Antenna (boomerang-shape)
CRT Chest Display Plates
Core-Funk Block Unit
Shoulder Groove Armor
Beam Saber Rack (retro-orange sheath)
Cosmic Funk Cannons
Star-Thruster Backpack
Hip Skirt – “CAN YOU DIG IT?” Plate
Knee Glide Assembly
Hover-Skid Foot Stabilizers
Style: blueprint line work on navy, cyan glow outlines, technical call outs.
Camera: orthographic front view, side inset.

## 中文译文（源站提供）
风格独特的胸部面板，散发出温暖的光芒；  
驾驶舱舱口处装有粗大的镀铬开关和旋转式旋钮；  
背包式推进器采用复古的星际飞船风格设计，配有洋红色和青绿色的等离子喷口；  
手臂上装有“Funk Cannon”武器（这是Captain HaHaa的标志性武器）；  
腿部装有反射宇宙色彩的镀铬面板；  
脚部稳定装置呈迷你悬浮滑橇形状；  
附有10张标签，详细说明每个独特的设计特点：  

- HaHaa标志天线（回旋镖形状）  
- CRT式胸部显示面板  
- Core-Funk功能模块  
- 肩部护甲  
- 光束剑收纳架（采用复古橙色护套）  
- 宇宙风格Funk Cannon武器  
- 星际推进器背包  
- 腰部装饰性配件，上面刻有“你喜欢吗？”字样  
- 膝盖滑动装置  
- 悬浮滑橇式脚部稳定装置  

设计风格：采用蓝绿色背景，线条轮廓带有海军蓝色调；技术细节部分采用文字标注方式进行说明。  
相机视角：正面正视图，侧面插图。

## 来源备注
- 源站作者：CaptainHaHaa
- 源站 UUID：0e94dd29a6
- 源站分类：08_图像分析信息拆解
