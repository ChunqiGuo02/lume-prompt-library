---
id: portrait-the-red-box-0407
title: "剧院奢华场景中年轻女性的写实肖像生成"
category: portrait
subcategory: "05_人物肖像与写实摄影"
aesthetic: null
tags: ["剧院", "优雅", "黑色", "白色", "手袋", "写真", "奢华", "暖光"]
recommended_models: 
  - banana-pro
recommended_params:
  aspect_ratio: "9:16"
  quality: null
  style_strength: null
sample_outputs: []
quality_tier: A
author: "ChunqiGuo02"
source: "Leader Prompt Gallery (https://www.leaderai.top/mid-api/lab/image_prompt/index.html)"
language: en
created_at: 2026-05-30
version: "1.0.0"
related_to: []
---

# 正文 prompt
{
  "image_generation_prompt": {
    "subject": {
      "description": "Young woman with long, straight light brown hair and a subtle, friendly smile.",
      "pose": "Sitting in a theater box seat, body turned slightly sideways away from the camera but head turned to face the viewer. Left arm resting casually on the red velvet box railing.",
      "clothing": "{"clothing" "Black satin dress with spaghetti straps and a low back, featuring a large black decorative bow at the lower back."}",
      "accessories": "Apple Watch with a metallic band on the left wrist, delicate bracelets, small earrings.",
      "expression": "Relaxed, happy, engaging eye contact."
    },
    "environment": {
      "location": "Grand historic European opera house interior (resembling the Vienna State Opera).",
      "architecture": "Multi-tiered private box seating (loges) featuring ornate gold Baroque/Rococo molding and stucco work. Deep red velvet curtains and wall coverings inside the boxes.",
      "background_details": "Several levels of balconies visible behind her, filled with other patrons in formal wear. Warm ambient lighting from wall sconces and theater chandeliers.",
      "foreground": "Red velvet cushioned railing and the back of a red theater seat."
    },
    "lighting_and_atmosphere": {
      "type": "Warm indoor artificial lighting mixed with soft camera flash.",
      "tone": "Golden, opulent, elegant, atmospheric.",
      "quality": "High dynamic range (HDR), evenly lit face, soft fall-off in the background."
    },
    "technical_specs": {
      "style": "Ultra-realistic, Photorealistic, Candid celebrity lifestyle photography.",
      "resolution": "8k UHD, highly detailed textures.",
      "camera_angle": "Eye-level, medium shot.",
      "aspect_ratio": "9:16 (Vertical/Long)",
      "lens_simulation": "35mm or 50mm portrait lens, aperture f/2.8 for slight depth of field blurring the background layers."
    },
    "full_combined_prompt": "A vertical ultra-realistic 8k photo of a young woman sitting in a lavish gold and red opera house box seat. She has long straight brunette hair, wearing a {"clothing description" "black backless satin dress with a large bow"}. She is smiling over her shoulder at the camera, arm resting on the red velvet railing. Background features ornate golden multi-tiered theater balconies filled with people, baroque architecture, warm chandelier lighting. High detailed texture, UHD, HDR, cinematic lighting, shot on 35mm lens --ar 9:16 --q 2 --v 6.0"
  }
}

## 中文译文（源站提供）
```json
{
  "image_generation_prompt": {
    "subject": {
      "description": "一位年轻女性，留着长直浅棕色头发，面带一丝友善的微笑。",
      "pose": "坐在剧院包厢座位上，身体略微侧向远离镜头，但头部转向面对观众。左臂随意地搭在红色天鹅绒包厢栏杆上。",
      "clothing": "{\"clothing\" \"黑色缎面吊带连衣裙，低背设计，下背部有一个大大的黑色装饰蝴蝶结。\"}",
      "accessories": "左手腕戴着金属表带的 Apple Watch，精致的手链，小耳环。",
      "expression": "放松、快乐，眼神交流富有吸引力。"
    },
    "environment": {
      "location": "宏伟的历史悠久的欧洲歌剧院内部（类似于维也纳国家歌剧院）。",
      "architecture": "多层私人包厢座位（loge），饰有华丽的金色巴洛克/洛可可式雕花和灰泥装饰。包厢内有深红色天鹅绒窗帘和墙面覆盖物。",
      "background_details": "她身后可见多层阳台，坐满了其他穿着正装的观众。壁灯和剧院吊灯散发出温暖的环境光。",
      "foreground": "红色天鹅绒软垫栏杆和红色剧院座椅的背面。"
    },
    "lighting_and_atmosphere": {
      "type": "温暖的室内人造光与柔和的相机闪光灯混合。",
      "tone": "金色、奢华、优雅、富有氛围感。",
      "quality": "高动态范围（HDR），面部光线均匀，背景柔和虚化。"
    },
    "technical_specs": {
      "style": "超现实主义、写实摄影、抓拍名人生活方式摄影。",
      "resolution": "8k UHD，高度精细的纹理。",
      "camera_angle": "平视视角，中景。",
      "aspect_ratio": "9:16（垂直/长构图）",
      "lens_simulation": "35mm 或 50mm 人像镜头，光圈 f/2.8，以实现轻微景深模糊背景层次。"
    },
    "full_combined_prompt": "一张垂直的超现实 8k 照片，一位年轻女性坐在奢华的金色和红色歌剧院包厢座位上。她留着长直的棕色头发，穿着一件 {\"clothing description\" \"黑色露背缎面连衣裙，带有一个大蝴蝶结\"}。她微笑着，越过肩膀看向镜头，手臂搭在红色天鹅绒栏杆上。背景是华丽的金色多层剧院阳台，坐满了人，巴洛克式建筑，温暖的吊灯照明。高细节纹理，UHD，HDR，电影级灯光，使用 35mm 镜头拍摄 --ar 9:16 --q 2 --v 6.0"
  }
}
```

## 来源备注
- 源站作者：TJ
- 源站 UUID：gOnvWsWYMx
- 源站分类：05_人物肖像与写实摄影
