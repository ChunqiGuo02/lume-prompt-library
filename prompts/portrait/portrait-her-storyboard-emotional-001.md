---
id: "portrait-her-storyboard-emotional-001"
title: "雨中离别情感叙事分镜"
category: portrait
subcategory: "09_故事分镜角色设定"
aesthetic: null
tags: ["雨天", "离别", "悲伤", "忧郁", "冷色调", "电影感", "孤独", "哭泣"]
recommended_models: 
  - banana-pro
recommended_params:
  aspect_ratio: "16:9"
  quality: null
  style_strength: null
sample_outputs: []
quality_tier: A
author: "ChunqiGuo02"
source: "Leader Prompt Gallery (https://www.leaderai.top/mid-api/lab/image_prompt/index.html)"
language: en
created_at: 2026-05-30
version: "1.0.0"
related_to: []
---

# 正文 prompt
Cinematic storyboard using Gemini Nano Banana Pro.
Prompt: Create a 9-panel cinematic storyboard set in a rainy, melancholic atmosphere. Show a young woman in a soaked dress standing beneath stone arches as a man walks away into the rain. Include close-ups of her emotional expression, a moment of their hands parting, shots of her crying against a pillar, and a final wide shot of her standing alone. Use soft, moody lighting, blue-gray tones, shallow depth of field, and dramatic raindrops. Panels should feel emotional, poetic, and film-like

## 中文译文（源站提供）
创作一个由 9 个画面组成的电影故事板，背景设定在阴雨绵绵、充满忧郁氛围的环境中。画面中应展示一位穿着湿透裙子的年轻女子站在石拱下，而一名男子则独自走进雨中。需要包含她的面部特写（展现她的情感状态）、他们手分开的瞬间、她靠在柱子上哭泣的镜头，以及她独自站立的最终全景画面。运用柔和、带有情绪色彩的灯光效果（蓝灰色调）、浅景深效果以及富有戏剧性的雨滴效果。整个故事板应充满情感张力、诗意感，并具有电影般的视觉风格。

## 来源备注
- 源站作者：Oogie
- 源站 UUID：f5289e1691
- 源站分类：09_故事分镜角色设定
