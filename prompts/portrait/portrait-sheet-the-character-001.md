---
id: "portrait-sheet-the-character-001"
title: "皮克斯风格角色表"
category: portrait
subcategory: "06_图文排版视觉传达"
aesthetic: null
tags: ["皮克斯", "角色", "设定", "草图", "卡通", "拟人", "手绘", "线稿"]
recommended_models: 
  - banana-pro
recommended_params:
  aspect_ratio: "16:9"
  quality: null
  style_strength: null
sample_outputs: []
quality_tier: B
author: "ChunqiGuo02"
source: "Leader Prompt Gallery (https://www.leaderai.top/mid-api/lab/image_prompt/index.html)"
language: en
created_at: 2026-05-30
version: "1.0.0"
related_to: []
---

# 正文 prompt
A character sheet of [detailed description of thecharacter], in the style of Pixar, featuring multiple poses and expressions from different angles, including concept art, pencil sketches, a full-body reference sheet, and a white background. The sheet includes wide shots, low-angle views, 3D renders, and Octane renderings, highly detailed result.

## 中文译文（源站提供）
这是一份角色资料表，采用了皮克斯的风格，详细描绘了该角色的外观特征。资料表包含了该角色从不同角度呈现的多种姿势和表情图，包括概念艺术图、铅笔草图、全身参考图以及白色背景背景下的各种视觉效果。内容涵盖了远景图、低角度视图、3D渲染图以及Octane软件生成的渲染图像，整体呈现出了极高的细节水平。

## 来源备注
- 源站作者：CharaspowerAI
- 源站 UUID：86ed4b7da5
- 源站分类：06_图文排版视觉传达
