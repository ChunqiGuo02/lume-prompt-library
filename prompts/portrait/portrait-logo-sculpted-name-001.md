---
id: "portrait-logo-sculpted-name-001"
title: "3D蒸汽朋克Logo"
category: portrait
subcategory: "03_创意广告品牌设计"
aesthetic: industrial-loft
tags: ["蒸汽朋克", "古铜色", "复古", "工业", "机械", "齿轮", "雕花", "做旧"]
recommended_models: 
  - banana-pro
recommended_params:
  aspect_ratio: "1:1"
  quality: null
  style_strength: null
sample_outputs: []
quality_tier: B
author: "ChunqiGuo02"
source: "Leader Prompt Gallery (https://www.leaderai.top/mid-api/lab/image_prompt/index.html)"
language: en
created_at: 2026-05-30
version: "1.0.0"
related_to: []
---

# 正文 prompt
3D sculpted logo of [LOGO NAME], in a steampunk style, made of aged brass and oxidized iron, with visible rivets, gears, and vintage mechanical elements, distressed and weathered surface, rich copper and bronze tones, engraved with Victorian-style filigree, retro-industrial design, soft spotlight lighting, neutral background, hyper-realistic render, ultra-high resolution, symmetrical composition

## 中文译文（源站提供）
这是一款采用蒸汽朋克风格的3D雕塑Logo，由古铜和氧化铁制成，配备了可见的铆钉、齿轮以及复古机械元件。其表面经过特殊处理，呈现出仿旧和风化的效果，色彩以丰富的铜色和青铜色调为主。Logo上刻有维多利亚风格的细工花纹，设计风格具有浓厚的复古工业风格。整个场景采用柔和的聚光灯照明，背景颜色保持中性，渲染效果极为逼真（超高清分辨率），整体构图呈现对称性。

## 来源备注
- 源站作者：MayorKingAI
- 源站 UUID：3027948f6b
- 源站分类：03_创意广告品牌设计
