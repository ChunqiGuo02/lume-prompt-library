---
id: "portrait-the-angle-grid-001"
title: "九宫格多角度可爱表情写真"
category: portrait
...
# 正文 prompt
Nano Banana Pro - 3×3 photo grid (Multi Angle Shot)
Prompt: https://t.co/buVX718Mjc
...
## 中文译文（源站提供）
生成一个3×3的照片网格。在所有面板中完全保留上传图像中的面部、发型和服装。这个人应该做出一个可爱、有趣、有点奇怪的表情和姿势，并且在所有9个面板上，相同的表情和姿态必须保持一致。每个面板应使用不同的相机角度。在不同的构图和框架中使用以下角度：1.高角度（自上而下）2.低角度（从下方）3.眼睛直视4.荷兰角（略微倾斜）5.特写低角度6.肩角7.侧面远射8.与前方成45度角9.轻微的鸟瞰角风格要求：•逼真、干净的照明•真实相机镜头渲染•无插图或卡通外观•所有图片中的服装、面部和发型都相
