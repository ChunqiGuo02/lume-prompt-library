---
id: "portrait-the-should-look-001"
title: "皮克斯风格3D卡通人物头像生成"
category: portrait
subcategory: "07_艺术插画创意风格"
aesthetic: null
tags: ["3D", "皮克斯", "卡通", "头像", "男生", "卷发", "微笑", "黑色"]
recommended_models: 
  - banana-pro
recommended_params:
  aspect_ratio: "2:3"
  quality: null
  style_strength: null
sample_outputs: []
quality_tier: B
author: "ChunqiGuo02"
source: "Leader Prompt Gallery (https://www.leaderai.top/mid-api/lab/image_prompt/index.html)"
language: en
created_at: 2026-05-30
version: "1.0.0"
related_to: ["portrait-the-should-look-002"]
---

# 正文 prompt
Create a high-quality 3D avatar of the person in the uploaded image with a cheerful, expressive face. The character should have a warm smile, bright eyes, and soft facial features that feel friendly and approachable. Render in a Pixar-style aesthetic with smooth textures, subtle skin shading, and slightly exaggerated proportions for a cute, animated look. Lighting should be soft and even, creating a clean studio look with gentle shadows for depth.

## 中文译文（源站提供）
根据上传图片中的人物，创建一个高质量的 3D 头像，使其面部表情愉悦、富有表现力。角色应拥有温暖的笑容、明亮的眼睛和柔和的面部特征，给人以友善和亲切的感觉。以皮克斯（Pixar）风格进行渲染，采用平滑的纹理、微妙的皮肤着色和略微夸张的比例，营造出可爱、动画化的外观。光线应柔和均匀，形成干净的影棚效果，并带有柔和的阴影以增加深度。

## 来源备注
- 源站作者：Harboriis
- 源站 UUID：eNW7yW54Vr
- 源站分类：07_艺术插画创意风格
