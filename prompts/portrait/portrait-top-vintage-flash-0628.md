---
id: portrait-top-vintage-flash-0628
title: "复古夜拍红发女郎倚粉色敞篷车，豹纹上衣配牛仔短裤，Y2K时尚"
category: portrait
subcategory: "05_人物肖像与写实摄影"
aesthetic: null
tags: ["红发", "粉色", "复古", "汽车", "豹纹", "夜景", "Y2K", "时尚"]
recommended_models: 
  - banana-pro
recommended_params:
  aspect_ratio: "3:4"
  quality: null
  style_strength: null
sample_outputs: []
quality_tier: A
author: "ChunqiGuo02"
source: "Leader Prompt Gallery (https://www.leaderai.top/mid-api/lab/image_prompt/index.html)"
language: en
created_at: 2026-05-30
version: "1.0.0"
related_to: []
---

# 正文 prompt
{
  "prompt": "A retro-inspired nighttime photoshoot of a red-haired woman leaning into a pink vintage convertible. She wears a {"top" "leopard-print off-shoulder top"} and white denim shorts, accessorized with a red necklace. Her long wavy hair cascades over one shoulder. The scene is lit with flash photography against a dark blue night sky and tree silhouettes, creating a bold contrast. Playful yet sultry mood, early 2000s aesthetic, glossy magazine editorial style.",
  "style": [
    "photorealistic",
    "Y2K aesthetic",
    "retro glamour",
    "editorial fashion"
  ],
  "camera": {
    "angle": "eye-level",
    "lens": "35mm",
    "lighting": "direct flash",
    "depth_of_field": "moderate"
  },
  "environment": "outdoor roadside setting at night with vintage car",
  "mood": "flirty, confident, nostalgic",
  "quality": "high detail, sharp focus, magazine-quality"
}

## 中文译文（源站提供）
```json
{
  "prompt": "一位红发女子斜倚在一辆粉色复古敞篷车上，拍摄复古风格的夜间照片。她身穿一件 {\"top\" \"豹纹露肩上衣\"} 和白色牛仔短裤，佩戴红色项链。她波浪状的长发披散在一侧肩上。场景在深蓝色夜空和树木剪影的映衬下，通过闪光灯摄影营造出鲜明的对比。整体氛围俏皮而性感，充满千禧年初（Y2K）美学，呈现光鲜的杂志编辑风格。",
  "style": [
    "超写实",
    "Y2K 美学",
    "复古魅力",
    "时尚编辑"
  ],
  "camera": {
    "angle": "平视视角",
    "lens": "35mm",
    "lighting": "直射闪光",
    "depth_of_field": "适中"
  },
  "environment": "夜间户外路边场景，配有老爷车",
  "mood": "轻佻、自信、怀旧",
  "quality": "高细节、锐利对焦、杂志品质"
}
```

## 来源备注
- 源站作者：Miz
- 源站 UUID：hSju6mkEMo
- 源站分类：05_人物肖像与写实摄影
