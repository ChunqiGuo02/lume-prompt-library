---
id: "portrait-the-cute-character-001"
title: "3D卡通角色多表情贴纸包生成"
category: portrait
subcategory: "16_表情包趣味拼图"
aesthetic: null
tags: ["可爱", "Q版", "3D", "卡通", "贴纸", "表情包", "亚洲女性", "米色"]
recommended_models: 
  - banana-pro
recommended_params:
  aspect_ratio: "1:1"
  quality: null
  style_strength: null
sample_outputs: []
quality_tier: A
author: "ChunqiGuo02"
source: "Leader Prompt Gallery (https://www.leaderai.top/mid-api/lab/image_prompt/index.html)"
language: en
created_at: 2026-05-30
version: "1.0.0"
related_to: []
---

# 正文 prompt
Please use the attached reference image as the base character.
Create a single high-resolution collage containing at least 20 cute sticker-style illustrations of the same female character. Ensure the face, skin tone, hairstyle, and overall appearance perfectly match the reference image.
Art Style: 【3D Pixar-style / Disney-style cartoon】, soft cinematic lighting, smooth pastel colors, large expressive eyes, rounded cute facial proportions, ultra-detailed 3D rendering.
Character Details:
– Lightly permed black hair
– Large expressive eyes
– Beige cardigan
– Cute, slightly chubby proportions
Expressions and Reactions (all distinct):
Happy smile, surprised, shocked, shy smile, angry, confused, crying, sleepy, winking, peace sign, blowing a kiss, holding a cell phone, thinking, covering face, holding coffee, covering mouth while laughing, annoyed, excited, affectionate expression, cute neutral pose (over 20 total).
Sticker Design:
– Each reaction as a separate sticker
– Clean white border on all stickers
– Soft drop shadow
– No text, no watermark
Layout:
– All stickers neatly arranged in one collage
– Balanced grid layout
– Soft beige/pastel-toned background
– Instagram/WhatsApp-style sticker pack design
Quality:
Ultra-high definition, high resolution, smooth 3D finish, professional-grade sticker pack style, lovingly expressive.

## 中文译文（源站提供）
请使用随附的参考图片作为基础角色。
创建一个高分辨率拼贴画，其中包含至少 20 张相同女性角色的可爱贴纸风格插图。确保面部、肤色、发型和整体外观与参考图片完美匹配。
艺术风格：【3D 皮克斯风格 / 迪士尼风格卡通】，柔和的电影光照，柔滑的蜡笔色，大而富有表现力的眼睛，圆润可爱的面部比例，超精细 3D 渲染。
角色细节：
– 微卷黑发
– 大而富有表现力的眼睛
– 米色开衫
– 可爱，略显丰满的比例
表情和反应（全部不同）：
开心的笑容、惊讶、震惊、害羞的笑容、生气、困惑、哭泣、困倦、眨眼、比耶手势、飞吻、拿着手机、思考、捂脸、拿着咖啡、捂嘴笑、恼怒、兴奋、深情的表情、可爱的中性姿势（总计超过 20 种）。
贴纸设计：
– 每个反应作为单独的贴纸
– 所有贴纸都有干净的白边
– 柔和的阴影
– 无文字，无水印
布局：
– 所有贴纸整齐地排列在一个拼贴画中
– 平衡的网格布局
– 柔和的米色/蜡笔色背景
– Instagram/WhatsApp 风格的贴纸包设计
质量：
超高清，高分辨率，流畅的 3D 效果，专业级贴纸包风格，充满爱意的表现力。

## 来源备注
- 源站作者：ケンイチ | AIスキルアカデミー『誰でもわかるAI活用術』
- 源站 UUID：FhDFNXAm6D
- 源站分类：16_表情包趣味拼图
