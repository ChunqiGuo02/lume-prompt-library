---
id: "portrait-celebrity-purple-silver-001"
title: "哥特朋克风紫黑格纹礼服造型，明星红毯写真"
category: portrait
subcategory: "05_人物肖像与写实摄影"
aesthetic: null
tags: ["紫色", "黑色", "女星", "朋克", "哥特", "甜酷", "时尚", "短裙"]
recommended_models: 
  - banana-pro
recommended_params:
  aspect_ratio: "2:3"
  quality: null
  style_strength: null
sample_outputs: []
quality_tier: A
author: "ChunqiGuo02"
source: "Leader Prompt Gallery (https://www.leaderai.top/mid-api/lab/image_prompt/index.html)"
language: en
created_at: 2026-05-30
version: "1.0.0"
related_to: []
---

# 正文 prompt
{
  "structured_prompt": {
    "shot_type": "Medium shot, red carpet celebrity photography, high-fashion editorial portrait",
    "subject_description": {
      "demographics": "Young female celebrity, early 20s, resemblance to {"celebrity likeness" "Sydney Sweeney"}",
      "physique": "Fit, slender figure, fair skin tone",
      "facial_features": "Soft almond-shaped blue eyes, rosy cheeks, soft pink lips, gentle expression"
    },
    "hair_and_makeup": {
      "hair_style": "Chin-length textured blonde bob, wavy and tousled, middle part, soft highlights",
      "makeup_look": "Fresh-faced glam, glowing skin, defined lashes, nude-pink lipstick, subtle blush"
    },
    "apparel": {
      "outfit_style": "Punk-couture, grunge-glam, Vivienne Westwood inspired aesthetic",
      "top_details": "Structured corset bodice in purple and black tartan plaid, multiple black leather buckle straps across the chest, silver hardware, silver gothic cross charm attached to center strap, ruffled neckline",
      "bottom_details": "Voluminous mini skirt with jagged handkerchief hem, mixed purple plaid and solid black fabric, purple satin bows, black lace tulle underlay/trim",
      "accessories": "Delicate diamond tennis necklace, silver rings, small stud earrings"
    },
    "pose_and_framing": {
      "pose": "Standing facing forward, hands resting gently on the upper thighs/skirt, confident but relaxed posture",
      "framing": "Waist-up to knee-up crop, centered composition"
    },
    "environment_and_lighting": {
      "background": "Solid matte {"background color" "black"} studio backdrop",
      "lighting_style": "Direct flash photography, high-contrast celebrity event lighting, sharp shadows, evenly lit subject"
    },
    "technical_specs": {
      "camera": "Canon EOS R5",
      "lens": "85mm portrait lens",
      "resolution": "8k, ultra-photorealistic, highly detailed textures",
      "render_style": "Unreal Engine 5, Octane Render, sharp focus, magazine quality"
    }
  }
}

## 中文译文（源站提供）
```json
{
  "structured_prompt": {
    "shot_type": "中景镜头，红毯名人摄影，时尚杂志肖像",
    "subject_description": {
      "demographics": "年轻女明星，20 岁出头，长相酷似 {\"celebrity likeness\" \"Sydney Sweeney\"}",
      "physique": "健美，苗条身材，白皙肤色",
      "facial_features": "柔和的杏仁状蓝色眼睛，红润的脸颊，柔和的粉色嘴唇，温柔的表情"
    },
    "hair_and_makeup": {
      "hair_style": "齐下巴的纹理感金色波波头，波浪卷曲凌乱，中分，柔和的挑染",
      "makeup_look": "清新妆容，光泽肌肤，睫毛分明，裸粉色口红，淡雅腮红"
    },
    "apparel": {
      "outfit_style": "朋克高级定制，垃圾摇滚魅力，Vivienne Westwood 风格美学",
      "top_details": "紫色和黑色格子呢结构化紧身胸衣，胸前多条黑色皮革搭扣带，银色五金件，中央搭扣上附有银色哥特式十字架吊坠，荷叶边领口",
      "bottom_details": "蓬松迷你裙，锯齿状手帕下摆，混合紫色格子和纯黑色面料，紫色缎面蝴蝶结，黑色蕾丝薄纱衬里/饰边",
      "accessories": "精致的钻石网球项链，银色戒指，小耳钉"
    },
    "pose_and_framing": {
      "pose": "站立面向前方，双手轻轻放在大腿/裙子上，自信而放松的姿势",
      "framing": "从腰部到膝盖的裁剪，居中构图"
    },
    "environment_and_lighting": {
      "background": "纯色哑光 {\"background color\" \"black\"} 摄影棚背景",
      "lighting_style": "直射闪光摄影，高对比度名人活动照明，锐利阴影，主体均匀受光"
    },
    "technical_specs": {
      "camera": "Canon EOS R5",
      "lens": "85mm 人像镜头",
      "resolution": "8k，超逼真，高度详细的纹理",
      "render_style": "Unreal Engine 5，Octane Render，锐利对焦，杂志品质"
    }
  }
}
```

## 来源备注
- 源站作者：Sudee🥀
- 源站 UUID：r1YgkyERfI
- 源站分类：05_人物肖像与写实摄影
