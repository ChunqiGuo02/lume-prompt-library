---
id: "landscape-the-main-bottom-001"
title: "硬装预览平面图转设计"
category: landscape
subcategory: "01_建筑室内空间设计"
aesthetic: null
tags: ["现代", "极简", "暖调", "原木", "客厅", "餐厅", "卧室", "书房"]
recommended_models: 
  - banana-pro
recommended_params:
  aspect_ratio: "1:1"
...
## 中文译文（源站提供）
根据上传的二维平面图，在单张图像中生成专业的室内设计展示板。布局：最终图像应该是拼贴，顶部有一个大的主图像，下面有几个小的图像。每个面板的内容：
1.主图像（上图）：主客厅的广角透视图，显示客厅和用餐区之间的连接。
2.小图片（左下）：主卧室的视图，重点放在床和窗户上。
3.小图片（中下）：家庭办公室/书房的视图。
4.小图像（右下）：显示家具布局的3D俯视平面图。整体风格：所有图像均采用一致的现代极简主义风格，温暖的橡木地板和灰白色墙壁。质量：逼真的渲染，柔和的自然光。
