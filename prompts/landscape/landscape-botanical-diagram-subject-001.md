---
id: "landscape-botanical-diagram-subject-001"
title: "复古植物学图谱设计"
category: landscape
subcategory: "08_图像分析信息拆解"
aesthetic: null
tags: ["复古", "植物", "手稿", "古籍", "棕色调", "人形", "骨架", "诡异"]
recommended_models: 
  - banana-pro
recommended_params:
  aspect_ratio: "3:2"
  quality: null
  style_strength: null
sample_outputs: []
quality_tier: B
author: "ChunqiGuo02"
source: "Leader Prompt Gallery (https://www.leaderai.top/mid-api/lab/image_prompt/index.html)"
language: en
created_at: 2026-05-30
version: "1.0.0"
related_to: []
---

# 正文 prompt
A botanical diagram of a [subject], illustrated in the style of vintage scientific journals. Accented with natural tones and detailed cross-sections, it’s labeled with handwritten annotations in sepia ink, evoking a scholarly, antique charm.
Check ATLs for in

## 中文译文（源站提供）
这是一幅[特定植物]的 botanical diagram（植物学插图），采用复古科学期刊的风格绘制。图中运用了自然的色彩基调，并详细展示了植物的横截面结构；所有标注均用棕褐色墨水手写而成，营造出一种学究气息与古朴美感。  
请在 ATLs 系统中查找相关信息。

## 来源备注
- 源站作者：Amira Zairi
- 源站 UUID：qKOSXf5aT5
- 源站分类：08_图像分析信息拆解
