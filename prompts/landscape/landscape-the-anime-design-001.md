---
id: "landscape-the-anime-design-001"
title: "日式动漫风格角色设计图"
category: landscape
subcategory: "10_卡通漫画电影角色"
aesthetic: japanese-zen-garden
tags: ["设计稿", "日式", "动漫", "废土", "面罩", "水彩", "暖色调", "金属"]
recommended_models: 
  - banana-pro
recommended_params:
  aspect_ratio: "3:2"
...
## 中文译文（源站提供）
将这张图片转化为一种受日本动漫机甲蓝图启发的风格化机械设计图。  
用精确、干净的线条和半逼真的赛璐珞阴影效果来呈现设计元素，使其看起来像出自动漫艺术册中的技术插图。  
将设计分解成各个部分（例如头部、手部、配件、全身），并以参考图的形式排列这些部分，展示不同的角度和视角。  
可以加入手写注释或日文字符作为概念说明。  
使用柔和的数字色彩（红色、灰色、金属色调），背景则采用松散的水彩风格绘制，呈现出动漫常见的风景元素，如悬崖、城市景观或草原废墟。  
最终的构图应显得像是一部科幻动画系列或漫画的官方设计页面。  
保持图片为横向格式（类似蓝图或角色表），在精确性与艺术风格之间取得平衡。
