---
id: "landscape-skin-the-stone-001"
title: "金发少女身着白色束腰连衣裙，倚靠石栏，户外花园光影写真"
category: landscape
subcategory: "05_人物肖像与写实摄影"
aesthetic: null
tags: ["金发", "白裙", "户外", "花园", "自然光", "人像", "时尚", "阳光"]
recommended_models: 
  - banana-pro
recommended_params:
  aspect_ratio: "9:16"
  quality: null
  style_strength: null
sample_outputs: []
quality_tier: A
author: "ChunqiGuo02"
source: "Leader Prompt Gallery (https://www.leaderai.top/mid-api/lab/image_prompt/index.html)"
language: en
created_at: 2026-05-30
version: "1.0.0"
related_to: ["landscape-skin-the-stone-002"]
---

# 正文 prompt
{
  "prompt_structure": {
    "subject": {
      "description": "Young woman in her 20s, with sun-kissed skin and long, wavy blonde hair blowing gently in the wind",
     "skin": "Fair skin tone",
      "facial_features": "Symmetrical face, soft makeup, glowing highlighter, glossy nude lips, defined eyebrows, piercing gaze looking directly at camera",
      "body": "Fit physique, collarbones visible"
    },
    "clothing": {
      "item": "White mini corset dress",
      "details": [
        "Ruffled shoulder straps",
        "Ruched bustier bodice",
        "Bubble hem skirt",
        "Voluminous gathered fabric",
        "Tight waistline"
      ],
      "accessories": "Delicate pearl choker necklace, small stud earrings, thin bracelet on left wrist"
    },
    "pose_and_action": {
      "pose": "Leaning back casually against a stone balustrade, hands resting on the stone railing behind her",
      "posture": "One leg slightly lifted/bent at the knee, relaxed but confident stance",
      "expression": "Sultry, confident, serene"
    },
    "environment": {
      "setting": "Outdoor garden or estate terrace",
      "background": "Lush green trees and foliage, slightly blurred (bokeh)",
      "foreground": "Weathered stone balustrade/railing with classic architectural details"
    },
    "lighting": {
      "type": "Bright natural sunlight ({"time of day" "Golden Hour"} or Mid-day)",
      "characteristics": "High contrast, hard shadows casting definition on the dress folds, strong rim lighting on hair, sun-drenched atmosphere"
    },
    "technical_specifications": {
      "camera_type": "DSLR",
      "lens": "85mm portrait lens",
      "aperture": "f/1.8",
      "focus": "Sharp focus on subject, depth of field blurring the background trees",
      "resolution": "8k, ultra-detailed",
      "film_stock": "Kodak Portra 400 aesthetic"
    },
    "style_modifiers": [
      "Ultra photorealistic",
      "Hyper-realistic skin texture",
      "Subsurface scattering",
      "Ray traced lighting",
      "Masterpiece",
      "Raw photo",
      "Fashion photography",
      "Cinematic lighting"
    ]
  }
}

## 中文译文（源站提供）
```json
{
  "prompt_structure": {
    "subject": {
      "description": "20 多岁的年轻女性，拥有阳光亲吻的肌肤和随风轻柔飘扬的金色波浪长发",
      "skin": "白皙肤色",
      "facial_features": "五官对称，淡雅妆容，高光提亮，水润裸色双唇，眉毛清晰，目光锐利直视镜头",
      "body": "健美身材，锁骨清晰可见"
    },
    "clothing": {
      "item": "白色迷你紧身胸衣连衣裙",
      "details": [
        "荷叶边肩带",
        "褶皱胸衣式上身",
        "泡泡裙摆",
        "蓬松褶皱面料",
        "紧身腰线"
      ],
      "accessories": "精致珍珠项圈，小巧耳钉，左手腕戴细手链"
    },
    "pose_and_action": {
      "pose": "随意地靠在石质栏杆上，双手放在身后的石栏杆上",
      "posture": "一条腿略微抬起/膝盖弯曲，姿态放松但自信",
      "expression": "性感、自信、宁静"
    },
    "environment": {
      "setting": "户外花园或庄园露台",
      "background": "郁郁葱葱的绿色树木和植物，略微模糊（散景）",
      "foreground": "风化的石质栏杆/扶手，带有经典的建筑细节"
    },
    "lighting": {
      "type": "明亮的自然阳光（{\"time of day\" \"Golden Hour\"} 或 中午）",
      "characteristics": "高对比度，硬朗的阴影在连衣裙褶皱上形成清晰的轮廓，头发上有强烈的轮廓光，阳光普照的氛围"
    },
    "technical_specifications": {
      "camera_type": "单反相机",
      "lens": "85mm 人像镜头",
      "aperture": "f/1.8",
      "focus": "主体对焦锐利，景深模糊背景树木",
      "resolution": "8k，超高细节",
      "film_stock": "柯达 Portra 400 胶片美学"
    },
    "style_modifiers": [
      "超逼真照片",
      "超写实皮肤纹理",
      "次表面散射",
      "光线追踪照明",
      "杰作",
      "原始照片",
      "时尚摄影",
      "电影级灯光"
    ]
  }
}
```

## 来源备注
- 源站作者：Iris
- 源站 UUID：JkXQquyQq3
- 源站分类：05_人物肖像与写实摄影
