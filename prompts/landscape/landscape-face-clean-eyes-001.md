---
id: "landscape-face-clean-eyes-001"
title: "老照片修复与美化服务"
category: landscape
subcategory: "11_图像编辑与修复增强"
aesthetic: null
tags: ["老照片", "修复", "肖像", "写实", "高清", "清晰", "夫妻", "人像"]
recommended_models: 
  - banana-pro
recommended_params:
  aspect_ratio: "3:4"
  quality: null
  style_strength: null
sample_outputs: []
quality_tier: A
author: "ChunqiGuo02"
source: "Leader Prompt Gallery (https://www.leaderai.top/mid-api/lab/image_prompt/index.html)"
language: en
created_at: 2026-05-30
version: "1.0.0"
related_to: []
---

# 正文 prompt
Restore your old and Damaged Photo
With Gemini Nano Banana 3.0
Prompt ⤵️
Luxury portrait of the uploaded image, 100% same face identity. Bright, clean exposure with soft studio lighting, balanced brightness, natural highlights on face and eyes, smooth shadow falloff. Ultra-realistic skin texture, sharp detailed eyes, natural hair strands, high-detail fabric. Clean elegant background with soft depth of field. Premium cinematic color grading, rich natural colors. Full-frame DSLR look, 85mm f/1.8, 8K ultra-realistic, no cartoon, no over-smoothing, no face change.

## 中文译文（源站提供）
上传的照片会呈现出奢华的肖像效果，面部细节100%保持原样：明亮的色调、干净细腻的曝光效果，搭配柔和的摄影棚灯光；亮度均匀，面部和眼部的高光自然自然，阴影过渡流畅。皮肤纹理极为逼真，眼睛轮廓清晰细致，头发纹理自然真实，衣物的质感也高度还原。背景干净优雅，具有柔和的景深效果。采用高级电影级色彩处理技术，呈现出丰富的自然色彩；整体效果呈现全画幅单反相机的质感（85mm f/1.8光圈），8K分辨率，既不过于卡通化，也不存在过度平滑处理或面部变形的问题。

## 来源备注
- 源站作者：Shreya Yadav
- 源站 UUID：bc1a0d222d
- 源站分类：11_图像编辑与修复增强
