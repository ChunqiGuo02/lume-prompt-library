---
id: "landscape-the-angle-all-001"
title: "九宫格多角度表情特写"
category: landscape
...
## 中文译文（源站提供）
生成一个 3×3 的照片网格。  
确保上传图片中的人物面部、发型和服装在所有格子中都得到完整保留。  
人物应呈现出可爱、有趣且略带奇特的表情和姿势，并且这种表情和姿势在所有 9 个格子中都保持一致。  
每个格子应使用不同的拍摄角度，构图和框架也要有所变化：  
1. 高角度（自上而下）  
2. 低角度（从下方）  
3. 与眼睛平行的正面视角  
4. 荷兰角度（略微倾斜）  
