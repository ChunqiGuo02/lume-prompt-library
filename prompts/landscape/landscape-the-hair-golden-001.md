---
id: "landscape-the-hair-golden-001"
title: "公主风写真：金发少女戴皇冠梳妆"
category: landscape
subcategory: "05_人物肖像与写实摄影"
aesthetic: null
tags: ["写实", "公主", "摄影", "长发", "金发", "皇冠", "紫色", "束腰"]
recommended_models: 
  - banana-pro
recommended_params:
  aspect_ratio: "1:1"
  quality: null
  style_strength: null
sample_outputs: []
quality_tier: A
author: "ChunqiGuo02"
source: "Leader Prompt Gallery (https://www.leaderai.top/mid-api/lab/image_prompt/index.html)"
language: en
created_at: 2026-05-30
version: "1.0.0"
related_to: []
---

# 正文 prompt
{
  "prompt_metadata": {
    "style": "Photorealistic, Cosplay Photography, Princess-core",
    "resolution": "4K, 8K, Ultra HD",
    "aspect_ratio": "Variable (Portrait or Square)",
    "lighting": "Soft indoor warm lighting, golden hour simulation"
  },
  "visual_elements": {
    "subject": {
      "description": "Young woman with fair skin and a soft, feminine aesthetic.",
      "hair": "Extremely long, flowing {"hair color" "golden blonde"} hair extending past the waist, styled straight with slight waves at the ends, featuring wispy curtain bangs framing the face.",
      "makeup": "Rosy flushed cheeks (heavy blush focus), glossy lips, soft eye makeup, glowing complexion."
    },
    "outfit": {
      "top": "{"corset color" "Lavender/purple"} brocade corset bodice with intricate floral or damask jacquard patterning. Features a sweetheart neckline trimmed with ruffled pink lace. Adorned with a large satin bow at the center front, combining baby blue and light pink ribbons.",
      "bottom": "Short, ruffled white or pale lavender skirt (visible in lower poses).",
      "accessories": "A sparkling, jeweled tiara/crown with silver setting and crystals sitting atop the head. Dangling pearl or crystal earrings."
    },
    "props": "Holding a round hairbrush or curling wand (visible in one variation).",
    "environment": "Indoor bedroom or dressing room setting, cream or white walls, soft ambient background, slightly candid vibe."
  },
  "technical_specifications": {
    "camera_settings": "Shot on 35mm lens, f/1.8 aperture for shallow depth of field.",
    "quality_tags": [
      "highly detailed",
      "photorealistic",
      "8k resolution",
      "HDR",
      "sharp focus on eyes",
      "cinematic lighting",
      "soft textures",
      "high fidelity"
    ]
  },
  "full_prompt_string": "A highly detailed, photorealistic 4k photo of a young woman cosplaying as Rapunzel. She has extremely long, flowing golden blonde hair reaching past her waist with curtain bangs and is wearing a sparkling jeweled tiara. She is dressed in a lavender brocade corset with intricate damask patterns, trimmed with pink ruffled lace and tied with baby blue and pink satin ribbons at the center. Her makeup features rosy cheeks and glossy lips. The lighting is soft and warm, imitating golden hour indoor bedroom lighting. The image quality is 8k, ultra-hd, cinematic, with sharp focus on the textures of the corset and hair."
}

## 中文译文（源站提供）
```json
{
  "prompt_metadata": {
    "style": "超写实摄影, 角色扮演摄影, 公主核心",
    "resolution": "4K, 8K, 超高清",
    "aspect_ratio": "可变（肖像或方形）",
    "lighting": "柔和的室内暖光, 模拟黄金时段"
  },
  "visual_elements": {
    "subject": {
      "description": "皮肤白皙、柔美女性化的年轻女子。",
      "hair": "极长、飘逸的 {\"hair color\" \"金金色\"} 头发，长及腰部以下，直发造型，发梢略带波浪，脸部周围有轻盈的法式刘海。",
      "makeup": "玫瑰色红润脸颊（着重腮红），光泽唇部，柔和眼妆，发光肤色。"
    },
    "outfit": {
      "top": "{\"corset color\" \"淡紫色/紫色\"} 锦缎束身胸衣，带有精致的花卉或大马士革提花图案。心形领口饰有荷叶边粉色蕾丝。胸前中央饰有一个大缎带蝴蝶结，结合了淡蓝色和浅粉色丝带。",
      "bottom": "短款、荷叶边白色或淡紫色短裙（在较低姿势中可见）。",
      "accessories": "一个闪闪发光的珠宝头饰/皇冠，银色镶嵌，水晶点缀，戴在头上。悬挂的珍珠或水晶耳环。"
    },
    "props": "手持一个圆形发刷或卷发棒（在某个变体中可见）。",
    "environment": "室内卧室或梳妆室场景，奶油色或白色墙壁，柔和的环境背景，略带随性氛围。"
  },
  "technical_specifications": {
    "camera_settings": "使用 35mm 镜头拍摄，f/1.8 光圈，实现浅景深。",
    "quality_tags": [
      "高度细节",
      "超写实",
      "8k 分辨率",
      "HDR",
      "眼睛锐利对焦",
      "电影级光照",
      "柔和纹理",
      "高保真"
    ]
  },
  "full_prompt_string": "一张高度细节、超写实的 4k 照片，一位年轻女子扮演长发公主。她拥有极长、飘逸的金色头发，长及腰部以下，带有法式刘海，头戴闪闪发光的珠宝头饰。她身穿淡紫色锦缎束身胸衣，带有精致的大马士革图案，饰有粉色荷叶边蕾丝，胸前中央系有淡蓝色和粉色缎带。她的妆容特点是玫瑰色脸颊和光泽唇部。光线柔和温暖，模仿黄金时段的室内卧室光线。图像质量为 8k，超高清，电影级，束身胸衣和头发的纹理清晰锐利。"
}
```

## 来源备注
- 源站作者：Mikasa
- 源站 UUID：zawGmU2rf9
- 源站分类：05_人物肖像与写实摄影
