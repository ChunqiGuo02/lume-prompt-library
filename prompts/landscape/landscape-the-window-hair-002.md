---
id: "landscape-the-window-hair-002"
title: "巴黎咖啡馆银发女子静思写真"
category: landscape
subcategory: "05_人物肖像与写实摄影"
aesthetic: null
tags: ["巴黎", "咖啡馆", "咖啡", "短发", "银灰", "米色", "风衣", "衬衫"]
recommended_models: 
  - banana-pro
recommended_params:
  aspect_ratio: "2:3"
  quality: null
  style_strength: null
sample_outputs: []
quality_tier: A
author: "ChunqiGuo02"
source: "Leader Prompt Gallery (https://www.leaderai.top/mid-api/lab/image_prompt/index.html)"
language: en
created_at: 2026-05-30
version: "1.0.0"
related_to: []
---

# 正文 prompt
A candid lifestyle portrait of a young woman with short {"hair color" "silver-gray"} hair sitting at a classic Parisian café by a large window. She is wearing a soft {"blouse color" "cream"} blouse with a delicate bow-tie neckline and high-waisted beige trousers, with a light trench coat draped casually over the back of her chair. She holds a white ceramic coffee cup with both hands, gazing thoughtfully out the window with a calm, elegant expression.
The setting features a cobblestone Paris street outside, Haussmann-style buildings with wrought-iron balconies, and subtle city details softly blurred in the background. Natural daylight streams through the window, creating gentle highlights and soft shadows on her face. Warm neutral color palette, minimalist composition, cozy European café atmosphere, shallow depth of field, realistic skin tones and fabric textures, cinematic lifestyle photography, editorial aesthetic, 50mm lens look, ultra-high resolution, photorealistic.

## 中文译文（源站提供）
一位年轻女士的真实生活肖像，她留着一头短款的{"hair color" "银灰色"}头发，坐在巴黎一家经典咖啡馆的大窗边。她身穿一件柔软的{"blouse color" "米色"}衬衫，领口系着精致的蝴蝶结，搭配高腰米色长裤，一件浅色风衣随意地搭在椅背上。她双手捧着一个白色陶瓷咖啡杯，若有所思地凝视窗外，神情平静而优雅。

背景是窗外鹅卵石铺就的巴黎街道，带有铸铁阳台的奥斯曼式建筑，以及背景中柔和模糊的城市细节。自然日光透过窗户洒进来，在她脸上形成柔和的高光和阴影。暖中性色调，极简构图，舒适的欧式咖啡馆氛围，浅景深，真实的肤色和织物质感，电影感生活摄影，杂志美学，50 毫米镜头效果，超高分辨率，写实主义。

## 来源备注
- 源站作者：K
- 源站 UUID：t59lvLn5KL
- 源站分类：05_人物肖像与写实摄影
