---
id: "landscape-skin-aesthetic-hair-001"
title: "时尚美妆杂志四格拼贴人像特写"
category: landscape
subcategory: "05_人物肖像与写实摄影"
aesthetic: null
tags: ["美妆", "拼接", "画报", "棕色", "裸妆", "帽子", "金饰", "短发"]
recommended_models: 
  - banana-pro
recommended_params:
  aspect_ratio: "2:3"
  quality: null
  style_strength: null
sample_outputs: []
quality_tier: A
author: "ChunqiGuo02"
source: "Leader Prompt Gallery (https://www.leaderai.top/mid-api/lab/image_prompt/index.html)"
language: en
created_at: 2026-05-30
version: "1.0.0"
related_to: []
---

# 正文 prompt
{
  "prompt_type": "beauty_editorial_collage",
  "layout": {
    "structure": "4-panel split-screen grid",
    "dividers": "Thick white diagonal borders",
    "style": "Modern magazine spread aesthetic"
  },
  "subject": {
    "demographic": "Young woman",
    "aesthetic": "Clean girl / K-beauty influence",
    "features": {
      "skin": "Flawless, dewy, 'glass skin' texture, high luminosity",
      "hair": "{"hair style" "Short brown bob cut with wispy bangs, smooth texture"}",
      "eyes": "Soft grey/hazel contacts"
    }
  },
  "makeup": {
    "style": "No-makeup makeup look",
    "details": [
      "Soft rosy blush high on cheeks",
      "Glossy pink lip tint",
      "Groomed natural fluffy brows",
      "Subtle mascara"
    ]
  },
  "attire_and_accessories": {
    "headwear": "Brown houndstooth pattern newsboy cap (Baker boy hat)",
    "jewelry": "Small delicate gold butterfly stud earrings, thin gold chain necklace"
  },
  "panel_composition": {
    "top_left": "Close-up 3/4 angle, focus on cheek highlight and earring",
    "top_right": "Direct front-facing portrait, symmetrical composition",
    "bottom_left": "Portrait with hat, 3/4 turn, looking at camera",
    "bottom_right": "Extreme close-up side profile, cropping into face"
  },
  "technical_specs": {
    "resolution": "8K",
    "lighting": "Soft commercial studio lighting, ring light reflection in eyes",
    "color_grading": "Desaturated background, warm skin tones, high contrast",
    "style": "High-end beauty photography, hyper-realistic"
  }
}

## 中文译文（源站提供）
```json
{
  "prompt_type": "beauty_editorial_collage",
  "layout": {
    "structure": "四格分屏网格",
    "dividers": "粗白色对角线边框",
    "style": "现代杂志版面美学"
  },
  "subject": {
    "demographic": "年轻女性",
    "aesthetic": "洁净女孩 / 韩系美妆风格",
    "features": {
      "skin": "无瑕、水润、‘玻璃肌’质感，高光泽度",
      "hair": "{\"hair style\" \"齐刘海棕色短波波头，发丝柔顺\"}",
      "eyes": "柔和的灰色/淡褐色美瞳"
    }
  },
  "makeup": {
    "style": "伪素颜妆容",
    "details": [
      "颧骨处涂抹柔和玫瑰色腮红",
      "光泽粉色唇釉",
      "精心修饰的自然蓬松眉毛",
      "若有似无的睫毛膏"
    ]
  },
  "attire_and_accessories": {
    "headwear": "棕色千鸟格报童帽 (贝克男孩帽)",
    "jewelry": "小巧精致的金色蝴蝶耳钉，细金链项链"
  },
  "panel_composition": {
    "top_left": "特写 3/4 角度，聚焦于脸颊高光和耳环",
    "top_right": "正面对称构图肖像",
    "bottom_left": "戴帽肖像，3/4 侧身，看向镜头",
    "bottom_right": "极近侧面特写，面部局部裁剪"
  },
  "technical_specs": {
    "resolution": "8K",
    "lighting": "柔和的商业影棚灯光，眼中可见环形灯反光",
    "color_grading": "背景去饱和，暖色调肤色，高对比度",
    "style": "高端美妆摄影，超现实主义"
  }
}
```

## 来源备注
- 源站作者：simeon-sanai
- 源站 UUID：j4hKoMXdIB
- 源站分类：05_人物肖像与写实摄影
