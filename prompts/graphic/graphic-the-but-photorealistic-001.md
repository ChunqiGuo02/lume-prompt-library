---
id: "graphic-the-but-photorealistic-001"
title: "生成多伦多棒球史手写照片"
category: graphic
subcategory: "06_图文排版视觉传达"
aesthetic: null
tags: ["棒球", "多伦多", "历史", "体育", "手写", "英文", "横格纸", "咖啡渍"]
recommended_models: []
recommended_params:
  aspect_ratio: "3:4"
  quality: null
  style_strength: null
sample_outputs: []
quality_tier: B
author: "ChunqiGuo02"
source: "Leader Prompt Gallery (https://www.leaderai.top/mid-api/lab/image_prompt/index.html)"
language: en
created_at: 2026-05-30
version: "1.0.0"
related_to: []
---

# 正文 prompt
a photorealistic, taken by phone photo of a handwritten essay in pencil, bold but elegant handwriting, but messy and somewhat uneven, on an 8.5x11 piece of lined paper, about the history of baseball in toronto. make sure there is variance in the writing in a very human way. give it a slight coffee stain on the top right corner

## 中文译文（源站提供）
这是一张用手机拍摄的照片，照片中的内容是一篇用铅笔手写的文章。字迹粗犷而优雅，但整体显得有些杂乱且不均匀，文章写在一张8.5×11英寸的横线纸上，内容是关于多伦多棒球的历史的。请确保字迹中存在一些自然的人为痕迹（即书写过程中的不规则性）。照片的右上角还有一点轻微的咖啡渍。

## 来源备注
- 源站作者：官方演示案例
- 源站 UUID：N9fO3Re8EP
- 源站分类：06_图文排版视觉传达
