---
id: "graphic-food-country-name-001"
title: "高端美食摄影：动感悬浮的异国风味料理"
category: graphic
subcategory: "04_创意脑洞特效合成"
aesthetic: japanese-zen-garden
tags: ["写实", "美食", "牛肉", "菠萝", "青柠", "香菜", "洋葱", "塔可"]
recommended_models: 
  - banana-pro
recommended_params:
  aspect_ratio: "3:4"
  quality: null
  style_strength: null
sample_outputs: []
quality_tier: A
author: "ChunqiGuo02"
source: "Leader Prompt Gallery (https://www.leaderai.top/mid-api/lab/image_prompt/index.html)"
language: en
created_at: 2026-05-30
version: "1.0.0"
related_to: []
---

# 正文 prompt
Ultra-high-quality, hyper-realistic commercial food photography of [{"country food name" "COUNTRY FOOD NAME"}], rendered in 8K detail with a (4:3 or 3:4) aspect ratio.

The food is shown in multiple premium visual variations, each highlighting texture, layers, and freshness in a cinematic way.

Food details:
– Authentic [{"country cuisine" "COUNTRY"}] cuisine presentation
– Clearly visible layers, fillings, or structure specific to [{"food type" "FOOD TYPE"}]
– Rich, natural colors and realistic textures

Motion & drama:
– Subtle mid-air elements (crumbs, ingredients, sauce, or spices)
– Natural gravity realism, frozen motion
– No mess, clean and controlled composition

Lighting & camera:
– Controlled studio lighting emphasizing texture and shine
– High-speed photography look
– Shallow to medium depth of field

Background:
– Solid or soft gradient background in [BACKGROUND COLOR]
– Clean, seamless, distraction-free

Style & finish:
– Luxury food advertising aesthetic
– Rich contrast, warm natural tones
– Ultra-sharp focus, no artificial glow

Negative prompt:
Text, logos, branding, hands, people, utensils, plates, tables, cartoon style, burnt food, overexposure, low-detail textures, artificial shine.

## 中文译文（源站提供）
超高质量、超逼真的商业美食摄影，拍摄 [{"country food name" "COUNTRY FOOD NAME"}]，以 8K 细节呈现，长宽比为 (4:3 或 3:4)。

食物以多种优质视觉效果呈现，每种都以电影般的方式突出质地、层次和新鲜度。

食物细节：
– 正宗的 [{"country cuisine" "COUNTRY"}] 美食呈现
– 清晰可见的 [{"food type" "FOOD TYPE"}] 特有的层次、馅料或结构
– 丰富自然的色彩和逼真的质感

动感与戏剧性：
– 细微的空中元素（碎屑、食材、酱汁或香料）
– 自然重力写实，凝固的动态
– 整洁，构图干净且受控

灯光与相机：
– 受控的影棚灯光，强调质感和光泽
– 高速摄影效果
– 浅到中等景深

背景：
– [BACKGROUND COLOR] 的纯色或柔和渐变背景
– 干净、无缝、无干扰

风格与修饰：
– 奢华美食广告美学
– 丰富对比度，温暖自然色调
– 超清晰对焦，无人工光晕

负面提示词：
文字、标志、品牌、手、人物、餐具、盘子、桌子、卡通风格、烧焦的食物、过度曝光、低细节纹理、人工光泽。

## 来源备注
- 源站作者：Aleena Amir
- 源站 UUID：46dyUzU3zu
- 源站分类：04_创意脑洞特效合成
