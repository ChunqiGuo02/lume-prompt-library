---
id: "graphic-prompt-devotee-lord-001"
title: "克里希纳神佑信徒场景"
category: graphic
subcategory: "04_创意脑洞特效合成"
aesthetic: null
tags: ["寺庙", "信徒", "男性", "黑天", "神像", "宗教", "印度教", "虔诚"]
recommended_models: 
  - banana-pro
recommended_params:
  aspect_ratio: "16:9"
  quality: null
  style_strength: null
sample_outputs: []
quality_tier: B
author: "ChunqiGuo02"
source: "Leader Prompt Gallery (https://www.leaderai.top/mid-api/lab/image_prompt/index.html)"
language: en
created_at: 2026-05-30
version: "1.0.0"
related_to: []
---

# 正文 prompt
Use this Prompt For Devotee & God Creation.
Gemini Ai Nano Banana 3 Pro ✨
Prompt ⤵️
Generate image of a smiling Indian young male devotee joining hands to statue of lord Krishna and sky blue hologram of happy lord Krishna is putting his hand over devotees shoulder in the temple, cinematic effect, pure devotional environment, 4k resolution, hindi text "कृष्ण सदा सहायते" under the image in fancy fonts

## 中文译文（源站提供）
生成一幅画面：一位面带微笑的印度年轻信徒与克里希纳神像双手合十，同时天空中出现一个蓝色的全息影像——快乐的克里希纳神正将手放在信徒的肩上；整个场景具有电影般的视觉效果，营造出浓厚的虔诚氛围。图片分辨率应为 4K，图片下方用花哨的字体显示印地语文字“कृष्ण सदा सहायते”（意为“克里希纳永远给予帮助”）。

## 来源备注
- 源站作者：🇮🇳 Jignesh Hindustani 🇮🇳
- 源站 UUID：eLMwHUYKJi
- 源站分类：04_创意脑洞特效合成
