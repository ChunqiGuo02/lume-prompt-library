---
id: "graphic-the-entering-create-001"
title: "光合作用教育信息图"
category: graphic
subcategory: "06_图文排版视觉传达"
aesthetic: null
tags: ["教育", "信息图", "光合作用", "植物", "太阳", "科学", "课本", "扁平"]
recommended_models: 
  - banana-pro
recommended_params:
  aspect_ratio: "16:9"
  quality: null
  style_strength: null
sample_outputs: []
quality_tier: B
author: "ChunqiGuo02"
source: "Leader Prompt Gallery (https://www.leaderai.top/mid-api/lab/image_prompt/index.html)"
language: en
created_at: 2026-05-30
version: "1.0.0"
related_to: []
---

# 正文 prompt
Create an educational infographic explaining [Photosynthesis] . Visual Elements : Illustrate the key components: The Sun, a green Plant, Water (H2O) entering roots, Carbon Dioxide (CO2) entering leaves, and Oxygen (O2) being released. Style : Clean, flat vector illustration suitable for a high school science textbook. Use arrows to show the flow of energy and matter. Labels : Label each element clearly in English .

## 中文译文（源站提供）
制作一张教育信息图，解释[光合作用]。视觉元素：说明关键组成部分：太阳、绿色植物、进入根部的水（H2O）、进入叶子的二氧化碳（CO2）和释放的氧气（O2）。风格：干净、平面的矢量插图，适合高中科学教科书。使用箭头显示能量和物质的流动。标签：用英语清楚地标记每个元素。

## 来源备注
- 源站作者：WeChat Article
- 源站 UUID：93074cfeb5
- 源站分类：06_图文排版视觉传达
