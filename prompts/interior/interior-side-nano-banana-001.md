---
id: "interior-side-nano-banana-001"
title: "轻松制作汽车切割视图"
category: interior
subcategory: "11_图像编辑与修复增强"
aesthetic: null
tags: ["汽车", "超跑", "白色", "透视", "剖面", "机械", "科技", "写实"]
recommended_models: 
  - banana-pro
recommended_params:
  aspect_ratio: null
  quality: null
  style_strength: null
sample_outputs: []
quality_tier: B
author: "ChunqiGuo02"
source: "Leader Prompt Gallery (https://www.leaderai.top/mid-api/lab/image_prompt/index.html)"
language: mixed
created_at: 2026-05-30
version: "1.0.0"
related_to: []
---

# 正文 prompt
Nano Banana、カットモデル図（透視図）みたいなのも簡単に作れますね
プロンプト:
Create a cutaway visualization of this car, show exterior intact on one side, and interior engine + seats exposed on the other side. Keep proportions accurate and details realistic. https://t.co/f6pD0DKWK4

## 中文译文（源站提供）
纳米香蕉（Nano Banana），以及类似切割模型图（透视图）这样的东西也可以轻松制作出来呢。  
提示：  
制作这辆汽车的剖面图，一侧保留完整的外部造型，另一侧展示内部发动机和座椅的构造。确保比例准确，细节逼真。  
参考链接：https://t.co/f6pD0DKWK4

## 来源备注
- 源站作者：old_pgmrs_will
- 源站 UUID：32441a91bc
- 源站分类：11_图像编辑与修复增强
