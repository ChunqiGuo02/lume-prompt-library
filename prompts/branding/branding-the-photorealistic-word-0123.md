---
id: branding-the-photorealistic-word-0123
title: "名字拼贴：皱纸与木纹"
category: branding
subcategory: "03_创意广告品牌设计"
aesthetic: null
tags: ["木纹", "撕纸", "蓝色", "白纸", "文字", "手工", "写实", "摄影"]
recommended_models: 
  - banana-pro
recommended_params:
  aspect_ratio: "3:2"
  quality: null
  style_strength: null
sample_outputs: []
quality_tier: B
author: "ChunqiGuo02"
source: "Leader Prompt Gallery (https://www.leaderai.top/mid-api/lab/image_prompt/index.html)"
language: en
created_at: 2026-05-30
version: "1.0.0"
related_to: []
---

# 正文 prompt
A photorealistic image of the word '[NAME]' spelled out using torn, highly crumpled pieces of white paper. Each letter is painted in bold [COLOR] on individual scraps, arranged loosely and unevenly, as if placed casually by hand, on a wooden table. The composition should convey a natural, handmade aesthetic with visible creases, shadows, and wood grain detail

## 中文译文（源站提供）
用撕碎的、高度皱巴巴的白纸拼出的单词“[NAME]”的照片级真实感图像。每个字母都用粗体[颜色]画在单独的碎片上，排列得松散而不均匀，就像用手随意放在木桌上一样。构图应传达一种自然的手工美学，带有可见的折痕、阴影和木纹细节

## 来源备注
- 源站作者：@umesh_ai
- 源站 UUID：6772b96bff
- 源站分类：03_创意广告品牌设计
