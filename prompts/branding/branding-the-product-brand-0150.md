---
id: branding-the-product-brand-0150
title: "品牌设计指南海报"
category: branding
subcategory: "03_创意广告品牌设计"
aesthetic: null
tags: ["品牌", "设计", "果汁", "简约", "橙色", "蓝色", "包装", "规范"]
recommended_models: 
  - banana-pro
recommended_params:
  aspect_ratio: "2:3"
...
## 中文译文（源站提供）
使用上传的产品图片创建一个垂直的9:16品牌设计指南海报。调整设计风格，以匹配产品的利基和视觉识别。用清晰、优雅的部分构建海报：（1）大型徽标显示和安全区使用，（2）以中心和突出显示的产品模型，（3）带有十六进制代码的主色调和次色调色板，（4）带有标题、副标题、正文字体样本和行距规格的排版指南，（5）品牌使用的图标或图形主题示例，（6）带有示例生活方式或工作室视觉效果的图像处理风格，（7）网格系统或布局规则，（8）包装模型和表面应用，（9）带有注释视觉效果的注意事项。使用极简主义的白色或柔和的中性背景，搭配结构化的布局分隔和阴影。结果必须视觉丰富、干净，适合印刷或数字品牌书籍。
